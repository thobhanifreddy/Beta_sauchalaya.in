...
function receivedMessage(event) {
  ...
  if (isEcho) {
    ...
    console.log("quick------>" + JSON.stringify(quickReply));
    if (quickReply.payload == "male") {
      quickReplyMale(senderID);
    } else if (
      quickReply.payload == "female" ||
      quickReply.payload == "pee" ||
      quickReply.payload == "potty"
    ) {
      quickReplyFemale(senderID);
    } else if (quickReply.payload == "yes" || quickReply.payload == "no") {
      sendTextMessage(senderID, "Thanks for your valuable review");
    } else {
      sendTextMessage(senderID, "Thanks go run to nearby toilet");
    }
    return;
  }
...
function sendLocationMessage(senderID, location) {
  console.log("loc--------> " + JSON.stringify(location));
  if (isReview) {
    isReview = false;
    var questions = [
      "Is this toilet open? क्या यह शौचालय खुला है?",
      "Is it Women-friendly? क्या यह महिलाओं के अनुकूल है?",
      "Is this clean toilet? क्या यह स्वच्छ शौचालय है?",
      "Is water facility available? क्या पानी की सुविधा उपलब्ध है?",
      "Are taps working properly? नल ठीक से काम कर रहे हैं?"
    ];
    ...
    var messageData = {
      ...
      message: {
        text: "Review the nearest toilet \n" + questions[index],
        quick_replies: [
    ...
    var title = location[0].title;
    var number = Math.floor(Math.random() * (10 - 5 + 1) + 5); //random number between 5-10
    var messageData = {
      recipient: { id: senderID },
      message: {
        text:
...
Total ` +
          number +
          ` public toilets are found near you. ` +
          `
Go to sauchalaya.in to know more`
      }
    };
  }
...
function sendReviewMessage(senderID) {
  isReview = true;
  var messageData = {
    recipient: { id: senderID },
    message: {
...
function sendDefaultMessage(senderID) {
  var messageData = {
    recipient: { id: senderID },
    message: {
      text: "I will help find you toilets around \n What's your gender??",
      quick_replies: [
...
function quickReplyMale(senderID) {
  var messageData = {
    recipient: { id: senderID },
    message: {
      text: "What are you finding toilet for?",
      quick_replies: [
        { content_type: "text", title: "Pee", payload: "pee" },
        { content_type: "text", title: "Potty", payload: "potty" }
      ]
    }
  };
  callSendAPI(messageData);
...
function quickReplyFemale(senderID) {
  var messageData = {
    recipient: { id: senderID },
    message: {
      text: "Share your current location",
